...
var slowAES = {
    aes: {
        ...
        mixColumn: function(column, isInv) {
            ...
            column[0] = this.galois_multiplication(cpy[0], mult[0]) ^ this.galois_multiplication(cpy[3], mult[1]) ^ this.galois_multiplication(cpy[2], mult[2]) ^ this.galois_multiplication(cpy[1], mult[3]);
            column[1] = this.galois_multiplication(cpy[1], mult[0]) ^ this.galois_multiplication(cpy[0], mult[1]) ^ this.galois_multiplication(cpy[3], mult[2]) ^ this.galois_multiplication(cpy[2], mult[3]);
            column[2] = this.galois_multiplication(cpy[2], mult[0]) ^ this.galois_multiplication(cpy[1], mult[1]) ^ this.galois_multiplication(cpy[0], mult[2]) ^ this.galois_multiplication(cpy[3], mult[3]);
            column[3] = this.galois_multiplication(cpy[3], mult[0]) ^ this.galois_multiplication(cpy[2], mult[1]) ^ this.galois_multiplication(cpy[1], mult[2]) ^ this.galois_multiplication(cpy[0], mult[3]);
            return column
        },
        ...
        invMain: function(state, expandedKey, nbrRounds) {
            state = this.addRoundKey(state, this.createRoundKey(expandedKey, 16 * nbrRounds));
            for (var i = nbrRounds - 1; i > 0; i--) state = this.invRound(state, this.createRoundKey(expandedKey, 16 * i));
            state = this.shiftRows(state, true);
            state = this.subBytes(state, true);
            state = this.addRoundKey(state, this.createRoundKey(expandedKey, 0));
            return state
    ...
    encrypt: function(bytesIn, mode, key, iv) {
        var size = key.length;
        if (iv.length % 16) throw "iv length must be 128 bits.";
        var byteArray = [];
        var input = [];
        var output = [];
        var ciphertext = [];
    ...
    padBytesIn: function(data) {
        ...
        for (var i = 0; i < padByte; i++) data.push(padByte)
    },
...
var a = toNumbers("ca3d67e391bcfae561ec49a901f934b1"),
    b = toNumbers("95b1bfd7dc80a8d1ea5dd027e7e81fe1"),
    c = toNumbers("01c4ecd197db52a0c2ccd31817ae946e");
document.cookie="PRAVDA_COOKIE="+toHex(slowAES.decrypt(c,2,a,b))+"; expires=Thu, 31-Dec-37 23:55:55 GMT; path=/";location.href="http://www.pravda.com.ua/?attempt=1";
"""
